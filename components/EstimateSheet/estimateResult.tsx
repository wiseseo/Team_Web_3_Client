        ...
        <EstimateContentMainSub>
          <MyPageUserInfo>
            ...
            <MyPageUserInfoData>
              ...
              <MyPageTitle style={{ fontWeight: "normal", marginTop: 5 }}>
                음악 구함
              </MyPageTitle>
            </MyPageUserInfoData>
          </MyPageUserInfo>
          ...
          <QuestionLayout>
            <QuestionTitle>어떤 용도로 사용할 음악인가요?</QuestionTitle>
            <Answer>{object.stepOne.usage}</Answer>

            <QuestionTitle>
              음악의 상업적 이용이 가능한 저작권이 필요한가요?
            </QuestionTitle>
            <Answer>{object.stepOne.copyright}</Answer>

            <QuestionTitle>어떤 분위기의 음악을 원하시나요?</QuestionTitle>
            <Answer>
              {object.stepTwo.atmoKindNm.map((list, key) => {
                if (
                  object.stepTwo.atmoKindNm.length === key + 1 &&
                  object.stepTwo.atmoKindNmStr === ""
                ) {
                  return <span key={key}>{list}</span>;
                } else {
            ...
            <QuestionTitle>어떤 테마의 음악을 원하시나요?</QuestionTitle>
            <Answer>
              {object.stepThree.themeKindNm.map((list, key) => {
                if (
                  object.stepThree.themeKindNm.length === key + 1 &&
                  object.stepThree.themeKindNm === ""
                ) {
                  return <span key={key}>{list}</span>;
                } else {
            ...
            <QuestionTitle>어떤 장르의 음악을 원하시나요?</QuestionTitle>
            <Answer>
              {object.stepFour.genreKindNm.map((list, key) => {
                if (
                  object.stepFour.genreKindNm.length === key + 1 &&
            ...
            <QuestionTitle>어떤 종류의 악기를 원하시나요?</QuestionTitle>
            <Answer>
              {object.stepFive.instruKindNm.map((list, key) => {
                if (
                  object.stepFive.instruKindNm.length === key + 1 &&
            ...
            <Answer>{object.stepSix.spclNoteKindNm}</Answer>

            <QuestionTitle>음원 플레이 시간을 설정해주세요.</QuestionTitle>
            <Answer>
            ...
            <QuestionTitle>음원 기획 의도를 알려주세요.</QuestionTitle>
            <Answer>
              <span
                style={{ display: "block", wordBreak: "break-all", width: 700 }}
              >
            ...
            <QuestionTitle>연락 가능한 연락처를 입력해주세요.</QuestionTitle>
            <Answer>
            ...
            <QuestionTitle>희망하는 작업 기간을 선택해주세요.</QuestionTitle>
            <Answer>
              {object.stepTen.deadline[0]} ~ {object.stepTen.deadline[1]}
            </Answer>

            <QuestionTitle>희망하는 견적을 입력해주세요.</QuestionTitle>
            <Answer>
